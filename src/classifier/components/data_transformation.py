...
import sys
import pandas as pd
from classifier.entity.artifacts import DataIngestionArtifacts, DataTransformationArtifacts
...
class DataTransformation:
    ...
    def imbalance_data_cleaning(self):
        ...
        try:
            ...
            imbalance_data = pd.read_csv(
                self.data_ingestion_artifacts.imbalance_data_file_path)
            ...
            logging.info(
                f"Exited the imbalance data_cleaning function and returned imbalance data {imbalance_data}"
            )
    ...
    def raw_data_cleaning(self):
        ...
        try:
            ...
            raw_data[raw_data[self.data_transformation_config.CLASS]
                     == 0][self.data_transformation_config.CLASS] = 1

            # replace the value of 0 to 1
            raw_data[self.data_transformation_config.CLASS].replace(
                {0: 1}, inplace=True
            )
            ...
            raw_data.rename(columns={
                            ...
                            inplace=True)
            logging.info(
                f"Exited the raw_data_cleaning function and returned the raw_data {raw_data}"
            )
            return raw_data
    ...
    def concat_data_cleaning(self, words):
        ...
        try:
            ...
            words = [
                word for word in words.split(' ') if words not in stopword
            ]
            words = " ".join(words)
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifacts:
        try:
            ...
            os.makedirs(
                self.data_transformation_config.DATA_TRANSFORMATION_ARTIFACTS_DIR, exist_ok=True)
